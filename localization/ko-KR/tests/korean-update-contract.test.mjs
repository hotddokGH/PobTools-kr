...
test('Korean release build disables every remote updater entry point', () => {
  const cmake = readFileSync(join(repositoryRoot, 'pob-zh-engine', 'CMakeLists.txt'), 'utf8');
  const header = readFileSync(join(repositoryRoot, 'pob-zh-engine', 'host', 'app_update.h'), 'utf8');
  const source = readFileSync(join(repositoryRoot, 'pob-zh-engine', 'host', 'app_update.cpp'), 'utf8');
...
test('Korean distribution defaults translation updates off', () => {
  const ini = readFileSync(join(repositoryRoot, 'pob-zh-engine', 'dist', 'pob-zh.ini'), 'utf8');
