...
test('preview workflow verifies offline unsigned executable, generated assets, package, provenance, and extracted ZIP', () => {
  ...
  const fontSelfTest = workflow.indexOf("$fontTest = Start-Process -FilePath $installedExe -ArgumentList '--font-coverage-selftest' -NoNewWindow -Wait -PassThru");
  ...
  assert.match(workflow, /\$fontTest\.ExitCode -ne 0/u);
  assert.match(workflow, /font coverage self-test failed/u);
  assert.match(workflow, /\$updateTest = Start-Process -FilePath \$installedExe -ArgumentList '--app-update-selftest' -NoNewWindow -Wait -PassThru/u);
  assert.match(workflow, /\$updateTest\.ExitCode -ne 0/u);
  assert.match(workflow, /offline updater self-test failed/u);
  assert.doesNotMatch(workflow, /& \(Join-Path \$installRoot 'pob-zh\.exe'\) --(?:font-coverage|app-update)-selftest/u);
