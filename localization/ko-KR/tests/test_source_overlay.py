...
class SourceOverlayTests(unittest.TestCase):
    ...
    def compatibility_recovery_row(self, **overrides):
        row = self.recovery_row()
        row["compatibilityPatchSha256"] = "A" * 64
    ...
    def test_scans_function_like_macro_argument_literals_with_file_scope_order(self):
        source = (
            '#define UPDATES(X) \\\n'
            '  X(kChecking, u8"업데이트 확인 중…") \\\n'
            '  X(kFound, u8"새 버전 발견 v")\n'
        )

        rows = scan_cpp_literals(Path("host/macros.h"), source.encode("utf-8"))

        self.assertEqual(
            [(row.function, row.decoded, row.occurrence_index) for row in rows],
            [("", "업데이트 확인 중…", 0), ("", "새 버전 발견 v", 1)],
        )

    def test_scans_object_like_macro_adjacent_components_with_exact_original_spans(self):
        source = '#define EXTERNAL_SEED u8"앞" /* keep */ u8"뒤"\n'

        row = scan_cpp_literals(Path("host/macros.h"), source.encode("utf-8"))[0]

        self.assertEqual("앞뒤", row.decoded)
        self.assertEqual(["앞", "뒤"], [part.decoded for part in row.components])
        self.assertEqual(
            [
                source.encode("utf-8")[part.start : part.end].decode("utf-8")
                for part in row.components
            ],
            ['u8"앞"', 'u8"뒤"'],
        )

    def test_apply_replaces_macro_argument_components_without_touching_macro_trivia(self):
        original = '#define EXTERNAL_SEED u8"設定" /* keep */ L"更新"\n'
        self.write("host/macros.h", original, newline="")
        ...
        self.assertEqual(
            '#define EXTERNAL_SEED u8"설정" /* keep */ L"업데이트"\n',
            self.read("host/macros.h"),
        )

    def test_normal_parser_literal_is_not_duplicated_by_macro_subparse(self):
        source = 'void Draw(){ Label(u8"설정"); }\n'

        rows = scan_cpp_literals(Path("host/ui.cpp"), source.encode("utf-8"))

        self.assertEqual(1, len(rows))
        self.assertEqual("설정", rows[0].decoded)

    def test_scans_valid_macro_expression_recovery_without_misclassifying_it(self):
        source = (
            '#define PT_VER_STR(x) #x\n'
            '#define PT_VERSION PT_VER_STR(MAJOR) "." PT_VER_STR(MINOR) "."\n'
        )

        rows = scan_cpp_literals(Path("host/app_version.h"), source.encode("utf-8"))

        self.assertEqual([".", "."], [row.decoded for row in rows])

    def test_malformed_macro_literal_fails_closed(self):
        source = '#define BAD_MACRO u8"설정\n'

        with self.assertRaises(CppParseError):
            scan_cpp_literals(Path("host/macros.h"), source.encode("utf-8"))

    def test_malformed_object_macro_expression_fails_closed_after_complete_literal(self):
        source = '#define BAD_MACRO u8"설정" +\n'

        with self.assertRaises(CppParseError) as caught:
            scan_cpp_literals(Path("host/macros.h"), source.encode("utf-8"))

        self.assertTrue(caught.exception.problems)

    def test_malformed_function_macro_call_fails_closed_after_complete_literal(self):
        source = '#define BAD_MACRO(X) X(u8"설정"\n'
    ...
    def test_exact_pinned_recovery_evidence_allows_valid_macro_boundary_fragment(self):
        repository_root = Path(__file__).resolve().parents[3]
        policy = json.loads(
            (repository_root / "localization/ko-KR/source-display-policy.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        for commit, role in (
            ...
            (self.PINNED_LOCALIZED, "localized"),
        ):
            with self.subTest(commit=commit):
                raw = subprocess.check_output(
                    ["git", "show", f"{commit}:pob-zh-engine/ui_api.cpp"],
                    cwd=repository_root,
                )
                ...
                self.assertIn("C++ exception:\n%s", [row.decoded for row in rows])

    def test_pinned_app_update_macro_inventory_includes_all_raw_korean_seed_literals(self):
        repository_root = Path(__file__).resolve().parents[3]
        raw = subprocess.check_output(
            [
                "git",
                "show",
                f"{self.PINNED_LOCALIZED}:pob-zh-engine/host/app_update.cpp",
            ],
            cwd=repository_root,
        )
        policy = json.loads(
        ...
        rows = scan_cpp_literals(
            ...
            source_commit=self.PINNED_LOCALIZED,
        )
        by_line = {row.line: row for row in rows}

        self.assertEqual("업데이트 확인 중…", by_line[98].decoded)
        self.assertEqual("최신 버전입니다 v", by_line[99].decoded)
        self.assertEqual("새 버전 발견 v", by_line[100].decoded)
        self.assertEqual("(현재 v", by_line[101].decoded)
        self.assertEqual(")", by_line[102].decoded)
        self.assertEqual(2, len(by_line[159].components))
        self.assertEqual([159, 160], [part.line for part in by_line[159].components])
        self.assertTrue(
            all(re.search(r"[가-힣]", part.decoded) for part in by_line[159].components)
        )
    ...
    def test_compatibility_recovery_variant_coexists_and_reports_its_patch(self):
        original = self.RECOVERY_FIXTURE
        self.write("host/reviewed.cpp", original, newline="")
        compatibility = self.compatibility_recovery_row()
        self.write("host/legacy.cpp", original, newline="")
        legacy = self.recovery_row(path="host/legacy.cpp")
        self.write_policy([self.recovery_row(), compatibility, legacy])
        mapping = self.mapping({"設定": self.entry("설정", "reviewed")})
        ...
        report = apply_overlay(
            ...
            self.policy,
            self.report,
            compatibility_patch_sha256=compatibility["compatibilityPatchSha256"],
        )

        self.assertEqual(report["issues"], [])
        self.assertIn("설정", self.read("host/legacy.cpp"))
        self.assertEqual(
            report["parseRecoveryEvidence"]["identities"],
            [
                {
                    "path": "host/legacy.cpp",
                    "sourceRole": "upstream",
                    "sourceCommit": self.PINNED_UPSTREAM,
                    "fileSha256": self.RECOVERY_FILE_SHA256,
                    "recoverySha256": self.RECOVERY_FINGERPRINT_SHA256,
                },
                {
                    ...
                    "fileSha256": self.RECOVERY_FILE_SHA256,
                    "recoverySha256": self.RECOVERY_FINGERPRINT_SHA256,
                    "compatibilityPatchSha256": "A" * 64,
                }
            ],
        )

    def test_compatibility_recovery_policy_rejects_malformed_and_duplicate_variants(self):
        compatibility = self.compatibility_recovery_row()
        invalid_lists = [
            [self.compatibility_recovery_row(compatibilityPatchSha256="a" * 64)],
            [self.compatibility_recovery_row(compatibilityPatchSha256="not-a-hash")],
            [{**compatibility, "extra": True}],
            [compatibility, {**compatibility}],
        ]
        for rows in invalid_lists:
            with self.subTest(rows=rows):
                with self.assertRaises(source_overlay.PolicyValidationError):
                    source_overlay.validate_parse_recovery_allowlist(rows)
    ...
    def test_cli_missing_compatibility_patch_writes_blocking_report_without_sources(self):
        original = 'void Draw(){ ImGui::Text(u8"設定"); }'
        self.write("host/ui.cpp", original)
        mapping = self.mapping({"設定": self.entry("설정", "reviewed")})
        missing_patch = self.root / "missing.patch"

        completed = subprocess.run(
            [
                sys.executable,
                str(LIB_ROOT / "source_overlay.py"),
                "apply",
                "--source-root",
                str(self.root),
                "--mapping",
        ...
        self.assertEqual(report["issues"][0]["code"], "INVALID_COMPATIBILITY_PATCH")
        self.assertEqual(self.read("host/ui.cpp"), original)
