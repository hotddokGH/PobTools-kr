...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    values.set(name, resolvePath(value));
  }
  return {
...
const dataRoot = join(arguments_.engineRoot, 'host', 'data');
const localeRoot = join(arguments_.engineRoot, 'dist', 'Data', 'poe1', 'ko-KR');
...
function officialEnglishMap() {
  const provenance = readJson(join(arguments_.reportRoot, 'display-closure', 'provenance.json'));
