...
@dataclass(frozen=True)
class _LiteralCandidate:
    """A parser literal expression expressed in phase-2 lexical byte spans.

    Tree-sitter exposes normal C++ expressions directly, but deliberately
    treats macro replacement lists as ``preproc_arg``.  Keeping the scanner's
    intermediate representation in byte spans lets a second parser view of a
    replacement list use the exact same literal decoder and original-source
    boundary map as an ordinary expression.
    """
...
class ParseRecoveryEvidence:
    ...
    recovery_sha256: str
    reason: str
    compatibility_patch_sha256: str | None = None
...
def validate_parse_recovery_allowlist(value: Any) -> tuple[ParseRecoveryEvidence, ...]:
    ...
    compatibility_field = "compatibilityPatchSha256"
    rows: list[ParseRecoveryEvidence] = []
    details: list[str] = []
    seen: dict[tuple[str, str, str, str], int] = {}
    for index, row in enumerate(value):
        field_names = set(row) if isinstance(row, dict) else set()
        if not isinstance(row, dict) or (
            field_names != required
            and field_names != required | {compatibility_field}
        ):
        ...
        reason = row["reason"]
        compatibility_patch_sha256 = row.get(compatibility_field)
        ...
        if (
            ...
            continue
        if compatibility_patch_sha256 is not None and (
            not isinstance(compatibility_patch_sha256, str)
            or re.fullmatch(r"[0-9A-F]{64}", compatibility_patch_sha256) is None
        ):
            details.append(
                f"parseRecoveryAllowlist[{index}] has an invalid compatibility patch SHA-256"
            )
            continue
        if not isinstance(reason, str) or not _normalized_policy_reason(reason):
            ...
            continue
        key = (path, source_role, source_commit, compatibility_patch_sha256 or "")
        if key in seen:
            details.append(
...
def _recovery_identity(row: ParseRecoveryEvidence) -> tuple[str, str, str, str, str, str]:
    return (
        row.path,
        row.source_role,
        row.source_commit,
        row.file_sha256,
        row.recovery_sha256,
        row.compatibility_patch_sha256 or "",
    )
...
def recovery_evidence_report(
    ...
    for row in sorted(rows, key=_recovery_identity):
        ...
        if row.compatibility_patch_sha256 is not None:
            identity["compatibilityPatchSha256"] = row.compatibility_patch_sha256
...
def _decode_literal_span(
    ...
    *,
    ...
    """Decode one phase-2 literal token addressed by lexical byte span."""
    token = lexical_text[start:end]
    if kind != "raw_string_literal":
        match = _REGULAR_OPEN.match(token)
        if match is None or not token.endswith(b'"'):
            raise ValueError(f"invalid string literal at byte {start}")
        encoded = token[match.end() : -1].decode("utf-8")
        return (
            _decode_escape_text(
                encoded,
                start=left_boundaries[start],
                end=right_boundaries[end],
                line=line,
                function=function,
                allow_legacy_nul=allow_legacy_nul,
            ),
    ...
    if not token.endswith(terminator):
        raise ValueError(f"unterminated raw string literal at byte {start}")
...
def _literal_candidate(
    ...
    *,
    ...
    if node.type == "concatenated_string":
        children = tuple(
            (child.type, offset + child.start_byte, offset + child.end_byte)
            for child in node.named_children
            if child.type in {"string_literal", "raw_string_literal"}
        )
        if not children:
            raise ValueError("concatenated string has no literal components")
        return _LiteralCandidate(
            node.type,
    ...
    return _LiteralCandidate(
        ...
        offset + node.start_byte,
        offset + node.end_byte,
        ((node.type, offset + node.start_byte, offset + node.end_byte),),
        function,
        function_scope,
    )


def _literal_candidates_in_tree(
    tree: Any,
    *,
    offset: int,
    function: str,
    function_scope: int,
) -> list[_LiteralCandidate]:
    rows: list[_LiteralCandidate] = []
...
def _macro_body_start(node: Node) -> int | None:
    """Return the replacement-list start for a macro definition node."""
    name_end = None
    name_start = None
    parameters_end = None
    for child in node.named_children:
        if child.type == "preproc_params":
            parameters_end = child.end_byte
    pending = [node]
    while pending:
        current = pending.pop()
        if current.type == "identifier" and (
            name_start is None or current.start_byte < name_start
        ):
            name_start = current.start_byte
            name_end = current.end_byte
        pending.extend(reversed(current.named_children))
    if name_end is None:
        return None
    return parameters_end if parameters_end is not None else name_end


def _macro_literal_candidates(
    node: Node,
    lexical_text: bytes,
    *,
    function: str,
    function_scope: int,
) -> tuple[list[_LiteralCandidate], list[tuple[int, int]]]:
    """Parse one replacement list without changing its original byte spans.

    Function-like replacement lists are a sequence of macro calls and are
    deliberately parsed as a translation-unit fragment. Object-like lists are
    parsed as the right-hand side of a synthetic declaration, which lets C++
    adjacency form one ``concatenated_string`` exactly as it would in source.
    The only allowed recovery in either fragment is a missing statement
    semicolon: macro replacement lists do not contain C++ statement
    terminators. The wrapper is parser input only; selected nodes are
    translated back to the untouched phase-2 byte range before decoding or
    replacement.
    """
    body_start = _macro_body_start(node)
    if body_start is None or body_start >= node.end_byte:
        return [], []
    body = lexical_text[body_start : node.end_byte]
    if b'"' not in body:
        return [], []
    if node.type == "preproc_function_def":
        ...
        wrapper = b"auto _source_overlay_macro_value = "
        fragment = wrapper + body + b";"
        prefix_length = len(wrapper)
    parsed = _PARSER.parse(fragment)
    recoveries = _ordered_recovery_nodes(parsed)
    invalid_recoveries = [
        item
        for item in recoveries
        if not (item.is_missing and item.type == ";")
    ]
    candidates = _literal_candidates_in_tree(
        parsed,
        offset=body_start - prefix_length,
        function=function,
        function_scope=function_scope,
    )
    if invalid_recoveries:
        details: list[tuple[int, int]] = []
        for item in invalid_recoveries:
            start = max(prefix_length, item.start_byte)
            end = max(start, item.end_byte)
            details.append(
                (body_start + start - prefix_length, body_start + end - prefix_length)
            )
        return candidates, details
    return candidates, []


def _literal_from_candidate(
    candidate: _LiteralCandidate,
    *,
    normalized_path: str,
    lexical_text: bytes,
    original_text: bytes,
    ...
    line = original_text.count(b"\n", 0, right_boundaries[candidate.start]) + 1
    decoded_parts: list[str] = []
    components: list[LiteralComponent] = []
    prefix = ""
    for index, (kind, start, end) in enumerate(candidate.components):
        component_line = original_text.count(b"\n", 0, right_boundaries[start]) + 1
        decoded, component_prefix = _decode_literal_span(
            kind,
            start,
            end,
            lexical_text,
        ...
        components.append(
            LiteralComponent(
                right_boundaries[start],
                right_boundaries[end],
                decoded,
                component_prefix,
                "raw" if kind == "raw_string_literal" else "regular",
                component_line,
            )
        )
        if index == 0:
    ...
    return Literal(
        normalized_path,
        right_boundaries[candidate.start],
        right_boundaries[candidate.end],
        "".join(decoded_parts),
        prefix,
        candidate.function,
        line,
...
def scan_cpp_literals(
    ...
    *,
    ...
    macro_nodes: list[tuple[Node, str, int]] = []
    ...
    def collect(node: Node, function: str, function_scope: int) -> None:
        ...
        if node.type in {"preproc_def", "preproc_function_def"}:
            macro_nodes.append((node, function, function_scope))
        if node.type in {"concatenated_string", "string_literal", "raw_string_literal"}:
    ...
    macro_candidates: list[_LiteralCandidate] = []
    macro_problem_spans: list[tuple[str, int, int]] = []
    macro_ranges: list[tuple[int, int]] = []
    for node, function, function_scope in macro_nodes:
        direct_candidates, direct_problems = _macro_literal_candidates(
            node,
            lexical_text,
            function=function,
            function_scope=function_scope,
        ...
        if direct_candidates:
            macro_candidates.extend(direct_candidates)
            macro_ranges.append(
                (
        ...
        if direct_problems:
            macro_problem_spans.extend(
                (function, start, end) for start, end in direct_problems
            )
    ...
    recovery_boundaries = list(range(len(text) + 1))

    def macro_recovery(node: Node, boundaries: list[int]) -> bool:
        start = boundaries[node.start_byte]
        end = boundaries[node.end_byte]
        return any(
            range_start <= start <= range_end
            and range_start <= end <= range_end
            for range_start, range_end in macro_ranges
        )
    ...
    full_recovery_nodes = raw_recovery_nodes if lexical_recovery_nodes else []
    relevant_evidence = [
        row
        ...
        and (
            row.compatibility_patch_sha256 is None
            or row.compatibility_patch_sha256 == compatibility_patch_sha256
        )
    ]
    file_sha256 = hashlib.sha256(text).hexdigest().upper()
    recovery_sha256 = _recovery_sha256(full_recovery_nodes)
    matching_evidence = [
        row
    ...
    compatibility_matches = [
        ...
        if row.compatibility_patch_sha256 == compatibility_patch_sha256
    ]
    if compatibility_patch_sha256 is not None and compatibility_matches:
        matching_evidence = compatibility_matches
    has_exact_recovery_evidence = (
        len(matching_evidence) == 1 and bool(full_recovery_nodes)
    )
    problem_nodes: list[Node] = []
    if full_recovery_nodes or relevant_evidence:
        if has_exact_recovery_evidence:
            if used_recovery_evidence is not None:
                used_recovery_evidence.add(matching_evidence[0])
        else:
            problem_nodes = [
                node
                for node in full_recovery_nodes
                if not macro_recovery(node, recovery_boundaries)
            ]
            if not problem_nodes and relevant_evidence:
                problem_nodes = [recovery_tree.root_node]
    if problem_nodes:
    ...
    lexical_problem_nodes = [
        node
        for node in lexical_recovery_nodes
        if not macro_recovery(node, original_boundaries)
    ]
    if lexical_problem_nodes and not raw_recovery_nodes:
        raise CppParseError(
            [
                {
                    "function": "",
                ...
                for node in lexical_problem_nodes
            ]
        )

    if macro_problem_spans and not has_exact_recovery_evidence:
        raise CppParseError(
            [
                {
                    "function": function,
                    "line": text.count(
                        b"\n", 0, original_boundaries[start]
                    )
                    + 1,
                    "startByte": original_boundaries[start],
                    "endByte": original_boundaries[end],
                    "fileSha256": file_sha256,
                    "recoverySha256": _recovery_sha256(lexical_recovery_nodes),
                }
                for function, start, end in sorted(macro_problem_spans)
            ]
        )
    ...
    unique_candidates = {
        (candidate.start, candidate.end): candidate for candidate in all_candidates
    }
    ordered_candidates = sorted(
        unique_candidates.values(), key=lambda candidate: (candidate.start, candidate.end)
    )
    final_occurrences: dict[tuple[int, int, int], int] = {}
    by_final_scope: dict[int, list[_LiteralCandidate]] = {}
    for candidate in ordered_candidates:
        by_final_scope.setdefault(candidate.function_scope, []).append(candidate)
    for function_scope, scoped_candidates in by_final_scope.items():
        for occurrence_index, candidate in enumerate(scoped_candidates):
            final_occurrences[(function_scope, candidate.start, candidate.end)] = occurrence_index

    rows: list[Literal] = []
    for candidate in ordered_candidates:
        occurrence_index = final_occurrences[
            (candidate.function_scope, candidate.start, candidate.end)
        ]
        try:
            rows.append(
                _literal_from_candidate(
                    candidate,
                    normalized_path=normalized_path,
                    lexical_text=lexical_text,
                    original_text=text,
                    left_boundaries=original_left_boundaries,
                    right_boundaries=original_boundaries,
...
def main(argv: list[str] | None = None) -> int:
    ...
    compatibility_patch_sha256 = None
    if arguments.compatibility_patch is not None:
        try:
            compatibility_patch_bytes = arguments.compatibility_patch.resolve().read_bytes()
        except OSError as error:
            report = _new_report(_source_files(arguments.source_root.resolve()))
            report["issues"].append(
                {"code": "INVALID_COMPATIBILITY_PATCH", "detail": str(error)}
            )
